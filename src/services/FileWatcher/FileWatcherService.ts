...
export class FileWatcherService extends EventEmitter {
  ...
  async startWatching(): Promise<void> {
    ...
    this.logger.log(
      `Started watching ${allowedPaths.length} allowed paths`,
      "success"
    );
  }
  ...
  private handleFileChange(
    ...
    // Filter by extension
    if (
      mergedConfig.includeExtensions.length > 0 &&
  ...
  private async hasFileActuallyChanged(filePath: string): Promise<boolean> {
    try {
      ...
      const hasChanged =
        !cached || cached.hash !== hash || cached.mtime !== stats.mtimeMs;
