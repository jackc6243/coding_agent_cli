...
import { ChunkStrategist } from "../types.js";
      ...
      if (section.match(/^#{1,6}\s+/)) {
        if (currentChunk) {
          chunks.push({
            ...
            endLine:
              this.getLineNumber(content, currentChunk) +
              currentChunk.split("\n").length,
            metadata: {},
          });
        }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (currentChunk.length >= TextChunker.MAX_CHUNK_SIZE) {
        ...
        const overlapLines = Math.min(
          3,
          Math.floor(TextChunker.OVERLAP_SIZE / 50)
        );
        currentChunk =
          lines.slice(Math.max(0, i - overlapLines), i + 1).join("\n") + "\n";
