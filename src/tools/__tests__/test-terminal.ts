...
async function testBasicBashCommand(terminalClient: TerminalToolClient) {
  ...
  if (toolCall.result.isError)
    throw new Error(`Command failed: ${toolCall.result.content}`);
  if (
    !toolCall.result.content?.toString().includes("Hello from terminal tool!")
  ) {
...
async function testBashCommandWithStderr(terminalClient: TerminalToolClient) {
  ...
  if (!toolCall.result) throw new Error("No result returned");
  if (toolCall.result.isError)
    throw new Error(`Command failed unexpectedly: ${toolCall.result.content}`);
  if (!toolCall.result.content?.toString().includes("STDERR")) {
    throw new Error("Expected STDERR section not found");
  }
}

async function testBashWithWorkingDirectory(
  terminalClient: TerminalToolClient
) {
  const toolCall = new ToolCall({
    toolName: "bash",
    clientName: "terminal",
...
exit 0`;
  ...
  try {
    ...
    if (!toolCall.result) throw new Error("No result returned");
    if (toolCall.result.isError)
      throw new Error(`Script failed: ${toolCall.result.content}`);

    const output = toolCall.result.content?.toString() || "";
    if (
      !output.includes("Script argument 1: hello") ||
      !output.includes("Script argument 2: world")
    ) {
...
`;
  ...
  try {
    ...
    const output = toolCall.result.content?.toString() || "";
    if (
      !output.includes("Python script running") ||
      !output.includes("Argument 1: test")
    ) {
...
async function testTimeout(terminalClient: TerminalToolClient) {
  ...
  if (!toolCall.result) throw new Error("No result returned");
  if (!toolCall.result.isError)
    throw new Error("Expected command to timeout but it didn't");

  // Accept any error for timeout test since different systems may have different timeout behaviors
  console.log(
    `⚠️  Command timed out as expected with message: ${toolCall.result.content}`
  );
}
...
async function testToolRegistration(terminalClient: TerminalToolClient) {
  ...
  if (!toolNames.includes("run_script"))
    throw new Error("run_script tool not registered");
...
async function runAllTests() {
  ...
  console.log(
    `📋 Available tools: ${terminalClient
      .getToolList()
      .map((t) => t.name)
      .join(", ")}`
  );

  await runTest("Tool Registration", () =>
    testToolRegistration(terminalClient)
  );
  await runTest("Input Validation", () => testInputValidation(terminalClient));
  await runTest("Basic Bash Command", () =>
    testBasicBashCommand(terminalClient)
  );
  await runTest("Bash Command with Error", () =>
    testBashCommandWithError(terminalClient)
  );
  await runTest("Bash Command with STDERR", () =>
    testBashCommandWithStderr(terminalClient)
  );
  await runTest("Bash with Working Directory", () =>
    testBashWithWorkingDirectory(terminalClient)
  );
  await runTest("Shell Script Execution", () =>
    testScriptExecution(terminalClient)
  );
  await runTest("Python Script Execution", () =>
    testPythonScript(terminalClient)
  );
