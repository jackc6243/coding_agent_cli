...
export class OpenAIEmbeddingService implements EmbeddingService {
  ...
  async embedRaw(raw: string): Promise<number[]> {
    try {
      ...
      this.logger.log("Failed to embed raw text", "error");
      throw error;
    }
  }

  async embedOneChunk(chunk: ChunkContent): Promise<Embedding> {
    try {
      ...
      this.logger.log("Failed to embed chunk", "error");
      throw error;
    }
  }

  async embedChunks(chunks: ChunkContent[]): Promise<Embedding[]> {
    try {
      ...
      this.logger.log("Failed to embed batch", "error");
      throw error;
    }
  }
}
