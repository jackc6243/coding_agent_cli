...
export class ChromaVectorStore implements IVectorStore {
  ...
  async eraseEmbeddings(chunks: Chunk[]): Promise<void> {
    if (!this.collection) {
      throw new Error("VectorStore not initialized. Call initialize() first.");
    }

    if (chunks.length === 0) {
      this.logger.log("No chunks to erase", "info");
      return;
    }

    try {
      const ids = chunks.map((chunk) => chunk.hash);
      await this.collection.delete({ ids });
      this.logger.log(`Erased ${chunks.length} embeddings`, "info");
    } catch (error) {
      this.logger.log("Failed to erase embeddings", "error");
      throw error;
    }
  }

  async eraseEmbedding(chunk: Chunk): Promise<void> {
    if (!this.collection) {
      throw new Error("VectorStore not initialized. Call initialize() first.");
    }

    try {
      await this.collection.delete({ ids: [chunk.hash] });
      this.logger.log(`Erased embedding for chunk ${chunk.hash}`, "info");
    } catch (error) {
      this.logger.log(
        `Failed to erase embedding for chunk ${chunk.hash}`,
        "error"
      );
      throw error;
    }
  ...
  async eraseFile(root: string): Promise<void> {
    if (!this.collection) {
      throw new Error("VectorStore not initialized. Call initialize() first.");
    }
    ...
    try {
      ...
      if (matchingIds.length > 0) {
        ...
        this.logger.log(
          `No vectors found with filePath starting with "${root}"`,
          "info"
        );
      }
    } catch (error) {
  ...
  async removeByFilePath(filePath: string): Promise<void> {
    ...
    try {
      ...
      if (results.ids && results.ids.length > 0) {
        ...
        this.logger.log(
          `Removed ${results.ids.length} chunks for ${filePath}`,
          "success"
        );
      }
    } catch (error) {
